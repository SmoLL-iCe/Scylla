...
bool IATSearch::searchImportAddressTableInProcess( DWORD_PTR startAddress, DWORD_PTR* addressIAT, DWORD* sizeIAT, bool advanced )
{
	...
	*sizeIAT = 0;
	...
	if ( !addressInIAT )
	{
		...
		LOGS_DEBUG( "searchImportAddressTableInProcess :: addressInIAT not found, startAddress " PRINTF_DWORD_PTR_FULL_S, startAddress );
...
bool IATSearch::findIATAdvanced( DWORD_PTR startAddress, DWORD_PTR* addressIAT, DWORD* sizeIAT )
{
	...
	*sizeIAT = (DWORD)( *( --iatPointers.end( ) ) - *( iatPointers.begin( ) ) + sizeof( DWORD_PTR ) );
	...
	LOGS( "IAT Search Adv: Found %d (0x%X) possible IAT entries.", iatPointers.size( ), iatPointers.size( ) );
	LOGS( "IAT Search Adv: Possible IAT first " PRINTF_DWORD_PTR_FULL_S " last " PRINTF_DWORD_PTR_FULL_S " entry.", *( iatPointers.begin( ) ), *( --iatPointers.end( ) ) );
			...
			if ( META_GET_FC( decomposerResult[ i ].meta ) == FC_CALL || META_GET_FC( decomposerResult[ i ].meta ) == FC_UNC_BRANCH )
			{
				if ( decomposerResult[ i ].size >= 5 )
				{
					...
					if ( decomposerResult[ i ].ops[ 0 ].type == O_DISP )
					{
						...
						distorm_format( &decomposerCi, &decomposerResult[ i ], &inst );
						LOGS_DEBUG( "%S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL_S, inst.mnemonic.p, inst.operands.p, decomposerResult[ i ].ops[ 0 ].type, decomposerResult[ i ].size, decomposerResult[ i ].disp );
