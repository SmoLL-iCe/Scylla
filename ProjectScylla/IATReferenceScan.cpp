	...
	if ( META_GET_FC( instruction->meta ) == FC_CALL || META_GET_FC( instruction->meta ) == FC_UNC_BRANCH )
	{
		if ( instruction->size >= 5 )
		{
			...
			if ( instruction->flags & FLAG_RIP_RELATIVE )
			{
				...
				LOGS_DEBUG( PRINTF_DWORD_PTR_FULL_S " " PRINTF_DWORD_PTR_FULL_S " %S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL_S, (DWORD_PTR)instruction->addr, ImageBase, inst.mnemonic.p, inst.operands.p, instruction->ops[ 0 ].type, instruction->size, INSTRUCTION_GET_RIP_TARGET( instruction ) );
			...
			if ( instruction->ops[ 0 ].type == O_DISP )
			{
				...
				distorm_format( &ProcessAccessHelp::decomposerCi, instruction, &inst );
				LOGS_DEBUG( PRINTF_DWORD_PTR_FULL_S " " PRINTF_DWORD_PTR_FULL_S " %S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL_S, (DWORD_PTR)instruction->addr, ImageBase, inst.mnemonic.p, inst.operands.p, instruction->ops[ 0 ].type, instruction->size, instruction->disp );
...
void IATReferenceScan::patchNewIat( DWORD_PTR stdImagebase, DWORD_PTR newIatBaseAddress, PeParser* peParser )
{
	...
	for ( std::vector<IATReference>::iterator iter = iatReferenceList.begin( ); iter != iatReferenceList.end( ); iter++ )
	{
		...
		//LOGS_DEBUG( "address %X old %X new %X",ref->addressVA, ref->targetPointer, newIatAddressPointer);
...
{
	LOGS_IMPORT( "------------------------------------------------------------" );
	LOGS_IMPORT( "ImageBase " PRINTF_DWORD_PTR_FULL_S " ImageSize %08X IATAddress " PRINTF_DWORD_PTR_FULL_S " IATSize 0x%X", ImageBase, ImageSize, IatAddressVA, IatSize );
	int count = 0;
	...
	for ( std::vector<IATReference>::iterator iter = iatDirectImportList.begin( ); iter != iatDirectImportList.end( ); iter++ )
	{
		...
		LOGS_IMPORT( "%04d AddrVA " PRINTF_DWORD_PTR_FULL_S " Type %s Value " PRINTF_DWORD_PTR_FULL_S " IatRefPointer " PRINTF_DWORD_PTR_FULL_S " Api %s %S", count, ref->addressVA, type, ref->targetAddressInIat, ref->targetPointer, apiInfo->module->getFilename( ), apiInfo->name );
		...
		if ( ( ref->targetAddressInIat < ImageBase ) || ( ref->targetAddressInIat > ( ImageBase + ImageSize ) ) ) //outside pe image
		{
			//if (isAddressValidImageMemory(ref->targetAddressInIat))
			{
				...
				if ( apiReader->getApiByVirtualAddress( ref->targetAddressInIat, &isSuspect ) != 0 )
				{
					...
					distorm_format( &ProcessAccessHelp::decomposerCi, instruction, &inst );
					LOGS_DEBUG( PRINTF_DWORD_PTR_FULL_S " " PRINTF_DWORD_PTR_FULL_S " %S %S %d %d - target address: " PRINTF_DWORD_PTR_FULL_S, (DWORD_PTR)instruction->addr, ImageBase, inst.mnemonic.p, inst.operands.p, instruction->ops[ 0 ].type, instruction->size, ref->targetAddressInIat );
...
void IATReferenceScan::patchDirectJumpTable( DWORD_PTR stdImagebase, DWORD directImportsJumpTableRVA, PeParser* peParser, BYTE* jmpTableMemory, DWORD newIatBase )
{
	...
	for ( std::set<DWORD_PTR>::iterator apiIter = apiPointers.begin( ); apiIter != apiPointers.end( ); apiIter++ )
	{
		...
		LOGS_IMPORT( "Relocation direct imports fix: Base RVA %08X Type HIGHLOW Offset %04X RelocTableEntry %04X", relocOffset & 0xFFFFF000, relocOffset & 0x00000FFF, ( IMAGE_REL_BASED_HIGHLOW << 12 ) + ( relocOffset & 0x00000FFF ) );
#endif
...
void IATReferenceScan::patchDirectImportInDump64( int patchPreFixBytes, int instructionSize, DWORD_PTR patchBytes, BYTE* memory, DWORD memorySize, bool generateReloc, DWORD patchOffset, DWORD sectionRVA )
{
	...
	else
	{
		...
		{
			DWORD relocOffset = sectionRVA + patchOffset + patchPreFixBytes;
			LOGS_IMPORT( "Relocation direct imports fix: Base RVA %08X Type DIR64 Offset %04X RelocTableEntry %04X", relocOffset & 0xFFFFF000, relocOffset & 0x00000FFF, ( IMAGE_REL_BASED_DIR64 << 12 ) + ( relocOffset & 0x00000FFF ) );
		}
